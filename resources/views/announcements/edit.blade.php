<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-blue-500 border border-gray-200 rounded-xl p-6">
            ...
            {{-- Form --}}
            <h2 class="font-bold text-3xl text-center text-white mb-6">Edit Announcement</h2>
            ...
            <form action="{{ route('announcements.update', $announcement->id) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="content" rows="6" required
                              class="w-full border-gray-300 focus:ring-0 focus:border-blue-500 rounded-md p-3 text-sm shadow-sm resize-none">{{ old('content', $announcement->content) }}</textarea>
                    @error('content')
                        <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
