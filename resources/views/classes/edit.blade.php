...
<div class="max-w-3xl mx-auto p-6">
    ...
    <div class="bg-gradient-to-br from-blue-600 to-blue-700 shadow-2xl rounded-2xl p-8 border border-blue-400/60 backdrop-blur-sm">
        <form method="POST" action="{{ route('activities.update', [$class->id, $activity->id]) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <h1 class="text-3xl font-extrabold text-center text-white tracking-wide mb-8 drop-shadow">
                Edit Activity – 
                <span class="text-yellow-300">{{ $activity->name }}</span>
            </h1>

            {{-- Activity Name --}}
            <div>
                <label class="block text-sm font-semibold text-yellow-200 mb-2">Activity Nams</label>
            ...
            {{-- Due Date --}}
            <div>
                <label class="block text-sm font-semibold text-yellow-200 mb-2">Due Date</label>
